...
from SinGAN.functions import ZeroPad2d
def train(opt,Gs,Zs,reals,NoiseAmp):
    ...
    while scale_num<opt.stop_scale+1:
        ...
        if (nfc_prev==opt.nfc):
            G_curr.set_state_dict(paddle.load('%s/%d/netG.pth' % (opt.out_,scale_num-1)))
            D_curr.set_state_dict(paddle.load('%s/%d/netD.pth' % (opt.out_,scale_num-1)))
...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,centers=None):
    ...
    for epoch in range(opt.niter):
        ...
        for j in range(opt.Dsteps):
            ...
            # errD_real.backward(retain_graph=False)
            D_x = -errD_real.item()
        ...
        if epoch % 500 == 0 or epoch == (opt.niter-1):
            ...
            paddle.save(z_opt, '%s/z_opt.pth' % (opt.outf))
...
def train_paint(opt,Gs,Zs,reals,NoiseAmp,centers,paint_inject_scale):
    in_s = paddle.full(reals[0].shape, 0, device=opt.device)
    scale_num = 0
    nfc_prev = 0
    ...
    while scale_num<opt.stop_scale+1:
        if scale_num!=paint_inject_scale:
            ...
            # paddle.save(Zs, '%s/Zs.pth' % (opt.out_))
            # paddle.save(Gs, '%s/Gs.pth' % (opt.out_))
            # paddle.save(reals, '%s/reals.pth' % (opt.out_))
            # paddle.save(NoiseAmp, '%s/NoiseAmp.pth' % (opt.out_))
...
def init_models(opt):
    #paddle.set_device("cpu" if opt.not_cuda else "gpu:0")
    #generator initialization:
    netG = models.GeneratorConcatSkip2CleanAdd(opt) #.to(opt.device)
    #netG.apply(models.weights_init) #怎么改
    if opt.netG != '':
    ...
    #netD.apply(models.weights_init) #怎么改
    if opt.netD != '':
...
def load_trained_pyramid_New(opt, mode_='train'):
    ...
    if(os.path.exists(dir)):
        ...
        while scale_num<opt.stop_scale+1:
            ...
            # assert os.path.exists('%s/Gs'% dir+str(scale_num)+'.pdparams')
            # G_curr.set_state_dict(paddle.load('%s/Gs'% dir+str(scale_num)+'.pdparams'))
            
            Gs.append(G_curr)
            # Zs.append(paddle.load('%s/Zs.pth' % dir+'/'+str(scale_num)))
            # reals = paddle.load('%s/reals.pth' % dir+'/'+str(scale_num))
            # NoiseAmp = paddle.load('%s/NoiseAmp.pth' % dir+'/'+str(scale_num))
