...
def reset_grads(model,require_grad):
    for p in model.parameters():
        ...
        p = paddle.to_tensor(p, dtype='float32', stop_gradient=False)
...
#         t = t.cuda()
#         #t = t.to(paddle.set_device('cuda'))
#     return t

def move_to_cpu(t):
    t = t.cpu()
    #t = t.to(paddle.set_device('cpu'))
    return t
...
def calc_gradient_penalty(netD, real_data, fake_data, LAMBDA, device=None):
    ...
    #interpolates = paddle.grad.Variable(interpolates, requires_grad=True) #怎么改
...
def save_networks(netG,netD,z,opt):
    paddle.save(netG.state_dict(), '%s/netG.pth' % (opt.outf))
    paddle.save(netD.state_dict(), '%s/netD.pth' % (opt.outf))
    paddle.save(z, '%s/z_opt.pth' % (opt.outf))
...
def load_trained_pyramid(opt, mode_='train'):
    ...
    if(os.path.exists(dir)):
        Gs = paddle.load('%s/Gs.pth' % dir)
        Zs = paddle.load('%s/Zs.pth' % dir)
        reals = paddle.load('%s/reals.pth' % dir)
        NoiseAmp = paddle.load('%s/NoiseAmp.pth' % dir)
    else:
...
#             G_curr.set_state_dict(paddle.load('%s'%dir+'/'+str(scale_num)+'/netG.pth'))
